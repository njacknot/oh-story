...
网文写作 skill 包，覆盖长篇与短篇网络小说的扫榜、拆文、写作、去AI味、封面图全流程。适配 Claude Code、OpenClaw，并通过项目化 `.oh-story-codex/` + `AGENTS.md` 约束 Trae SOLO / Cloud Agents 使用本地 skill。
...
| Skill | 触发 | 说明 |
|:------|:-----|:-----|
| `story-setup` | `/story-setup` `/准备写书` | 环境部署 · `.oh-story-codex`/AGENTS.md/hooks/rules/agents/CLAUDE.md 一键部署（已有配置安全合并） |
...
`/story-setup` 会把当前 oh-story-codex 包复制到项目根目录 `.oh-story-codex/`，并生成 `AGENTS.md`。这样即使运行环境不能安装全局 skill，SOLO 或其他 Cloud Agents 也能按项目内的本地 skill 执行。

底层可执行脚本：`skills/story-setup/scripts/deploy-projectized.sh <项目根目录> [oh-story-codex根目录]`。
...
如果你已经在写作项目中运行过 `/story-setup`，升级 skill 后请在项目根目录重新运行一次 `/story-setup`。本 fork 的部署标记为 `agents_version` v9，包含 upstream v8 的 reviewer 路径修复，并额外部署项目化 skill 包、`chapter-editor` 和 Codex 原生子代理。

本版重点合并 upstream v0.6.8，并保留 fork 的项目化与子代理能力：
