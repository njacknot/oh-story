...
如果当前工作目录是 `/Users/jack/Codex/FanqieNovel`，或项目根存在 `AGENTS.md` 且说明为 FanqieNovel，必须使用这个项目的每日短篇目录结构：

```text
stories/YYYY/MM/YYYY-MM-DD_slug/
├── meta.md
├── 选题策划.md
├── 设定.md
├── 小节大纲.md
...
1. 不在项目根或任意松散 `{短篇标题}/` 下写正文。
2. 如果还没有故事目录，先用 `./scripts/new_story.sh "slug" "YYYY-MM-DD"` 创建。
3. 如果故事目录里已有 `选题策划.md`，优先把它当作写作 brief，不再追问题材和情绪。
4. `设定.md` 和 `小节大纲.md` 必须先于 `正文.md` 完成。
5. 草稿只写入 `正文.md`；创作必要的字数统计、格式自检、改稿记录按项目既有文件写入 `自检.md` 或 `修改记录.md`，不得附加到 `正文.md`。
6. 每日目标是一篇可继续推进的短篇：即使不能一次写完，也必须把当天项目推进到明确阶段，并在故事状态文件中更新 `Status`。
...
- **用模块组装**：铺垫段、升级段、高潮/兑现段各有成熟写法，不要重新发明。参考 genre-writing-formulas.md 对应题材
...
#### Codex 适配：story-architect

构思阶段默认由 Codex 主会话直接完成。只有当前环境明确提供可用的多代理工具、且用户要求拆分任务时，才把「项目目录 + 情绪目标 + 题材方向」交给等价的 story-architect 子任务；没有子代理能力时不要停顿，主会话继续执行。
...
### 核心爆点/转折
- 爆点类型：{反转/报应兑现/关系升温/认知更新/心死抽离/真相揭露}
- 是否反转：{是/否；无反转题材说明替代机制}
- 爆点内容：{一句话描述}
- 铺垫/期待线索：{至少 3 个铺垫点；无反转题材可写报应、甜点、关系或共鸣线索}

### 情绪设计
- 开头情绪：{}（强度 {1-10}）
- 中段情绪：{}（强度 {1-10}）
- 高潮情绪：{}（强度 {1-10}，峰值维持 ≥2 节）
- 结尾情绪：{}（强度 {1-10}）
- 高潮不要骤降：高潮前 1 节开始升温，高潮节达到峰值，高潮后 1 节维持峰值不骤降
...
- 角色：{主角/关键关系人/反派}
- 压力触发：{最怕被什么击中}
- 默认反应：{压住/反击/逃走/讨好/装冷静/转移话题}
- 身体信号：{压力上来时最先出现的动作}
- 语言变化：{话变短/变刺/变软/开始撒谎/重复某句话}
- 失控边界：{什么情况下会做出错误选择}
```
...
#### Codex 适配：character-designer

设计任务完成后，默认由 Codex 主会话补全角色设定和语言风格档案。只有当前环境明确提供可用的多代理工具、且用户要求拆分任务时，才把「项目目录 + 人设速写 + 关系」交给等价的 character-designer 子任务；没有子代理能力时主会话继续执行。
...
- 反转型埋入至少 3 个反转线索，分散在不同小节；无反转题材改为埋报应、甜点、关系变化或共鸣线索
...
#### 第四段：高潮/反转/兑现（占全文 10-15%）

- 反转型在一节内完成揭示，不拖延；无反转题材在一节内完成关键兑现或关系/认知转折
...
精修阶段默认由 Codex 主会话直接执行去AI味、格式合规和一致性检查；如用户明确调用 `/story-deslop` 或 `/story-review`，可把它们作为后续外部检查。不要因为没有 `.claude/agents` 或子代理工具而中断短篇创作。
