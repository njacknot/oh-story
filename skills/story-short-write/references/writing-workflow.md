...
- 核心爆点验证 → 反转型加载 `reversal-toolkit.md`，无反转题材加载对应题材公式或 `emotional-methods.md` ✅/❌
...
3. **确定揭露/兑现方式**：反转型从 villain-and-reveal.md 4 种真相揭露机制中选一种；无反转题材确定报应、甜点、关系升温、心死抽离或共鸣回扣的兑现方式
4. **补人物压力反应卡**：只给主角、关键关系人、反派填写，写入 `设定.md`。每张卡只保留 5 行：压力触发 / 默认反应 / 身体信号 / 语言变化 / 失控边界
5. **编写 小节大纲.md**：每节 1 行，格式如下：
   `主事件 | 子事件×3-5 | 情绪 | 读者新获知什么 | 钩子 | 伏笔/物件 | 动静 | 对话密度 | 目标字数`
   - 子事件必须功能明确（推动剧情/铺垫核心爆点/升级情绪），禁止装饰性子事件
...
6. **核心爆点验证**（outline 写完后、写作前必须验证）：
   - 反转型回答：读者在第几节知道真相？用「揭穿节号 ÷ 全文总节数」计算位置百分比
     - < 50% → 不是反转，是延迟揭露（读者已知真相太早，后续缺少震撼）
     - 50-60% → 灰区，需审视反转力度是否足够（前文误导线索是否充分）
     - > 60% → 反转位置合理（读者等得够久，冲击力最大化）
   - 反转型检查：至少有 1 条读者被成功误导的假线索（不是读者猜到后等角色追上来）
   - 反转型检查：反转前是否铺垫了 ≥3 条指向真相的线索（线索要隐蔽，不是明示）
   - 反转型验证：找一个人（或自我模拟）只看 outline，能否在反转前猜到真相？猜到 = 失败
   - 无反转题材验证：读者期待是否被连续抬高？关键兑现是否出现在 70%-90%？兑现是否回扣前文 ≥3 个期待线索？如果只是平铺完成事件 = 失败
7. **伏笔回查清单**（outline 写完后、写作前检查）：
   - 列出结尾所有关键元素（物件/信息/角色/关系变化）
   - 每个结尾元素在前文有 ≥1 次铺垫？没有 = 天降，必须补铺垫
   - 结构物件三现完整？（第 1 现在铺垫段，第 2 现在高潮/兑现段，第 3 现在结尾段）
...
│       └── 写作手法.md
└── {标题}/
    ├── 设定.md      # 核心框架 + 人设 + 核心爆点铺垫 + 结构物件（含三现追踪表）+ 反派设计 + 对标摘要
...
- 设定.md 维护人物压力反应卡；高压场景写作前先看对应角色卡，避免所有人物都用同一种身体反应或沉默/冷笑模板
